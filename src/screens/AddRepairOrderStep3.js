...
const AddRepairOrderStep3 = ({
    ...
    const [isGarageId, setIsGarageId] = useState(
        route?.params?.userVehicleDetails?.garage_id
    );
    const [isUserId, setIsUserId] = useState(
        route?.params?.userVehicleDetails?.user_id
    );
    const [isVehicleId, setIsVehicleId] = useState(
        route?.params?.userVehicleDetails?.vehicle_id
    );
    const [isName, setIsName] = useState(
        route?.params?.userVehicleDetails?.name
    );
    const [isEmail, setIsEmail] = useState(
        route?.params?.userVehicleDetails?.email
    );
    const [isPhoneNumber, setIsPhoneNumber] = useState(
        route?.params?.userVehicleDetails?.phone_number
    );

    // Vehicle Fields
    const [isBrandName, setIsBrandName] = useState(
        route?.params?.userVehicleDetails?.brand_name
    );
    const [isModelName, setIsModelName] = useState(
        route?.params?.userVehicleDetails?.model_name
    );
    const [isVehicleRegistrationNumber, setIsVehicleRegistrationNumber] =
        useState(
            route?.params?.userVehicleDetails?.vehicle_registration_number
        );
