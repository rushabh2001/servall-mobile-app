...
import { View , Text, StyleSheet, Keyboard, ActivityIndicator, TouchableOpacity, Pressable, Image, RefreshControl, FlatList } from 'react-native';
import { connect } from 'react-redux';
import { colors } from '../constants';
import { API_URL, WEB_URL } from '../constants/config';
import { Button, Divider, Modal, Portal, TextInput, List, Searchbar } from 'react-native-paper';
...
const EditVehicle = ({ navigation, userToken, route, selectedGarageId, selectedGarage, user }) => {
    ...
    useEffect(() => {
        if(selectedVehicleData != 0) {
            ...
            if(selectedVehicleData?.brand?.name) setIsBrandName(selectedVehicleData.brand?.name);
            if(selectedVehicleData?.vehicle_model?.model_name) setIsModelName(selectedVehicleData?.vehicle_model?.model_name);
            ...
            if(selectedVehicleData?.insurance_provider?.name) setIsInsuranceProviderName(selectedVehicleData.insurance_provider.name);
