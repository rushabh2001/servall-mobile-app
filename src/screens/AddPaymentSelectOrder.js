...
const AddPaymentSelectOrder = ({navigation, userToken, selectedGarageId, navigator  }) => {
    ...
    const getOrderDetails = async (orderId) => {
        try {
            const res = await fetch(`${API_URL}fetch_vehicle_data?id=${orderId}`, {
                method: 'GET',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
                                    ...
                                    <View style={styles.cards}>
                                        <View style={styles.upperInfo}>
                                            ...
                                            <View style={{right: 30, top: 35,position: 'absolute'}} >
                                                ...
                                                <Icon onPress={() => {setSelectedOrderId(item.id); refRBSheet.current.open();}} type={"MaterialCommunityIcons"} style={{right: 5, top: 8,position: 'absolute'}} name={'dots-vertical'} size={22}  color={colors.gray} />
                                            </View>
                                            <View>
